...
Algorithm = K_Means(k = 4,data = Customers_data,p=1) #모델 구현
Algorithm.Clustering()#Clustering 실행
Annual = [ [] for _ in range(Algorithm.k+1)] #값을 볼 수 있는 형태로 변환
Spending = [ [] for _ in range(Algorithm.k+1)]
for i in range(Algorithm.N): #같은 군집끼리 값을 모은다
    Annual[Algorithm.Cluster[i]].append(Annual_Income[i])
    Spending[Algorithm.Cluster[i]].append(Spending_Score[i])


for i in range(1,Algorithm.k+1): # k개의 군집 출력
    plt.scatter(Annual[i], Spending[i],s=50,marker = 'o', label = f'cluster{i}')
    plt.scatter(Algorithm.standard[i-1][2],Algorithm.standard[i-1][3],label=f"{i}'s Centroids",marker='x')#중심점
